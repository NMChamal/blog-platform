    ...
    <div className="ml-4 pl-4 border-l-2 border-gray-200">
      ...
      {!showEditForm ? (
        ...
        <form onSubmit={handleEditSubmit} className="mt-2">
          ...
          <div className="flex justify-end space-x-4 mt-2">
            <button type="button" onClick={() => setShowEditForm(false)} className="px-4 py-2 font-bold text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">
              Cancel
            </button>
      ...
      {!isDeleted && !showEditForm && (
        <div className="flex items-center space-x-4 mt-2">
          {isAuthenticated && (
            <button onClick={() => setShowReplyForm(!showReplyForm)} className="text-indigo-600 hover:underline">
              Reply
            </button>
          )}
          {(isAuthenticated && userId === comment.user._id) && (
            <button onClick={() => setShowEditForm(true)} className="text-indigo-600 hover:underline">
              Edit
            </button>
          )}
          {(isAuthenticated && userId === comment.user._id || role === 'admin') && (
      ...
      <div className="mt-4">
        {comment.children?.map((child) => (
          <Comment key={child._id} comment={child} postId={postId} mutateComments={mutateComments} />
        ))}
      </div>
    </div>
  );
