...
const PostPage = () => {
  ...
  const { data: postData, error: postError, isLoading: postIsLoading, mutate: mutatePost } = useApi(`/api/posts/${id}`);
  const { data: likeData, error: likeError, isLoading: likeIsLoading, mutate: mutateLike } = useApi(isAuthenticated ? `/api/posts/${id}/like` : null);
  const { data: likeCountData, error: likeCountError, isLoading: likeCountIsLoading, mutate: mutateLikeCount } = useApi(`/api/posts/${id}/likes/count`);
  const { data: commentsData, error: commentsError, isLoading: commentsIsLoading, mutate: mutateComments } = useApi(`/api/posts/${id}/comments`);
    ...
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        {userId === postData.data.author._id && (
          <Link to={`/posts/${id}/edit`} className="px-4 py-2 font-bold text-white bg-indigo-600 rounded-md hover:bg-indigo-700">
            Edit Post
          </Link>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center space-x-4">
        <button onClick={handleLike} className="flex items-center space-x-2 text-gray-600 hover:text-red-500">
          {likeData?.data.liked ? <HeartIconSolid className="w-6 h-6 text-red-500" /> : <HeartIconOutline className="w-6 h-6" />}
      ...
      <div className="mt-8">
        ...
        {isAuthenticated ? (
          <form onSubmit={handleCommentSubmit} className="mb-4">
            ...
            />
            <button type="submit" className="mt-2 px-4 py-2 font-bold text-white bg-indigo-600 rounded-md hover:bg-indigo-700">
              Post Comment
            </button>
          </form>
        ) : (
        ...
        <div className="space-y-4">
          {commentsData.data.length > 0 ? (
            commentsData.data.map((comment: any) => (
              <Comment key={comment._id} comment={comment} postId={id!} mutateComments={mutateComments} />
            ))
          ) : (
            <p>No comments yet.</p>
          )}
